...
- **Dynamic**: `GET /api/v1/openapi.json` and `GET /api/v1/openapi.yaml` — rebuilt from the live registry on every request, so it can never be stale.
...
Every `/api/v1/*` handler carries an `@document(...)` decorator that:
- Validates the request body against a Pydantic `*In` model
- Serializes the response from a Pydantic `*Out` model (or passes raw `Response` objects through unchanged)
- Records an `EndpointDoc` entry the spec builder reads

Schemas live in `auto_a11y/web/api/schemas/` (one module per §5.x section); the decorator and builder live in `auto_a11y/web/api/openapi/`.

### 10.3 Drift gates

Three independent gates prevent the spec from drifting from the running code:

1. **`scripts/generate_openapi.py --check`** — fails if `docs/api/openapi.yaml` does not match the freshly-built spec. Wired into the pre-commit hook and the `openapi_check` CI job.
2. **`openapi-spec-validator`** — validates the YAML against the OpenAPI 3.1 schema in CI.
3. **`tests/api/test_openapi_contract.py::test_every_api_v1_route_is_documented`** — fails if any `/api/v1` route lacks an `EndpointDoc` in the registry (allowlist enumerates known undocumented routes; new ones fail the gate).
