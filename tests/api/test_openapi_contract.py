...
# Single-entry allowlist for the GET /test-results/compare route, which
# uses query parameters (``?a=<id>&b=<id>``) for the diff selection. The
# ``@document`` decorator currently accepts a JSON ``request`` body or a
# multipart ``request_form`` but not a query-parameter shape, so this
# route can't be documented today without inventing a body it doesn't
# have. Drop the entry once ``@document`` grows a ``request_query``
# option (then wrap the route in ``auto_a11y/web/routes/api.py``).
#
# Phase-3 follow-ups (F1a → F1c) closed the previous 23 entries: F1a
...
# supervisors).
#
# A new undocumented route lands today as a hard test failure with a
# precise diff, not a silently-tolerated gap.
_KNOWN_UNDOCUMENTED: frozenset[tuple[str, str]] = frozenset({
    ("GET", "/test-results/compare"),
})
