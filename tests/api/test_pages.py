"""Tests for the ``/api/v1/websites/<website_id>/pages`` and
``/api/v1/pages/<page_id>`` REST endpoints.

Coverage:

- Anonymous requests get 401 with a Problem-Details body.
- Authenticated callers without the right role get 403.
- Missing resources get 404.
- Round-trip: POST → GET → PATCH → PUT → GET → DELETE → 404.
- Validation errors carry RFC 7807 ``errors`` arrays with field paths.
- List endpoint paginates by cursor and filters by status.
- Idempotent create: posting the same URL twice returns the same id.
- url is immutable on PUT.

Tests skip when ``mongod`` is not reachable. CI provisions one; local
runs need ``mongod`` listening on the default port (or ``MONGODB_URI``
set).
"""
from __future__ import annotations

import os
import uuid
from collections.abc import Generator, Iterator
from typing import Any

import pytest
from flask import Flask
from flask.testing import FlaskClient
from flask_login import LoginManager
from pymongo import MongoClient
from pymongo.errors import ConnectionFailure, ServerSelectionTimeoutError

from auto_a11y.core.database import Database
from auto_a11y.models.app_user import AppUser, UserRole
from auto_a11y.models.project import Project, ProjectStatus
from auto_a11y.models.website import Website


def _mongo_uri() -> str:
    return os.environ.get("MONGODB_URI", "mongodb://localhost:27017/")


@pytest.fixture(scope="module")
def mongo_check() -> None:
    """Skip the whole module unless mongod is reachable."""
    client: MongoClient[dict[str, Any]] = MongoClient(
        _mongo_uri(), serverSelectionTimeoutMS=2000
    )
    try:
        client.admin.command("ping")
    except (ConnectionFailure, ServerSelectionTimeoutError) as exc:
        client.close()
        pytest.skip(f"mongod not available at {_mongo_uri()}: {exc}")
    client.close()


@pytest.fixture
def database(mongo_check: None) -> Generator[Database, None, None]:
    """Per-test ephemeral test database; dropped on teardown."""
    db_name = f"auto_a11y_api_test_{uuid.uuid4().hex}"
    db = Database(_mongo_uri(), db_name)
    yield db
    db.client.drop_database(db_name)
    db.close()


@pytest.fixture
def flask_app(database: Database) -> Flask:
    """Slim Flask app: api_bp + Flask-Login + db, no migrations or runners."""
    app = Flask(__name__)
    app.config["TESTING"] = True
    app.config["SECRET_KEY"] = "test-secret-deterministic-for-tests-only"
    app.config["WTF_CSRF_ENABLED"] = False
    setattr(app, "db", database)

    login_manager = LoginManager()
    login_manager.init_app(app)

    def load_user(user_id: str) -> AppUser | None:
        return database.get_app_user(user_id)

    login_manager.user_loader(load_user)

    from auto_a11y.web.api import register_api_error_handlers
    from auto_a11y.web.routes.api import api_bp

    app.register_blueprint(api_bp, url_prefix="/api/v1")
    register_api_error_handlers(app)
    return app


@pytest.fixture
def client(flask_app: Flask) -> FlaskClient:
    return flask_app.test_client()


def _make_user(database: Database, *, role: UserRole, email: str) -> AppUser:
    user = AppUser.create(email=email, password="x", role=role)
    user_id = database.create_app_user(user)
    saved = database.get_app_user(user_id)
    assert saved is not None
    return saved


def _make_project(database: Database) -> Project:
    project = Project(
        name=f"Project {uuid.uuid4().hex[:8]}",
        description="",
        status=ProjectStatus.ACTIVE,
        config={},
    )
    project_id = database.create_project(project)
    saved = database.get_project(project_id)
    assert saved is not None
    return saved


def _make_website(database: Database, project: Project) -> Website:
    project_id = project.id
    assert project_id is not None
    website = Website(
        project_id=project_id,
        url=f"https://example-{uuid.uuid4().hex[:8]}.test/",
        name="Test website",
    )
    website_id = database.create_website(website)
    saved = database.get_website(website_id)
    assert saved is not None
    return saved


@pytest.fixture
def admin_user(database: Database) -> AppUser:
    """A superadmin who passes every project_role_required check."""
    user = _make_user(database, role=UserRole.ADMIN, email="admin@example.test")
    user.is_superadmin = True
    database.update_app_user(user)
    refreshed = database.get_app_user(user.id) if user.id else None
    assert refreshed is not None
    return refreshed


@pytest.fixture
def website(database: Database) -> Website:
    return _make_website(database, _make_project(database))


@pytest.fixture
def login(client: FlaskClient) -> Iterator[Any]:
    def _login(user: AppUser) -> None:
        with client.session_transaction() as sess:
            sess["_user_id"] = str(user.id)
            sess["_fresh"] = True

    yield _login


def _valid_create_body() -> dict[str, Any]:
    return {
        "url": f"https://example.test/page-{uuid.uuid4().hex[:8]}",
        "title": "Page one",
        "priority": "normal",
    }


# ---------------------------------------------------------------------------
# Auth matrix
# ---------------------------------------------------------------------------


def test_anonymous_list_returns_401(
    client: FlaskClient, website: Website
) -> None:
    response = client.get(f"/api/v1/websites/{website.id}/pages")
    assert response.status_code == 401
    assert response.headers["Content-Type"] == "application/problem+json"
    body = response.get_json()
    assert body["type"].endswith("/unauthorized")


def test_anonymous_create_returns_401(
    client: FlaskClient, website: Website
) -> None:
    response = client.post(
        f"/api/v1/websites/{website.id}/pages", json=_valid_create_body()
    )
    assert response.status_code == 401


def test_authenticated_non_member_returns_403(
    client: FlaskClient,
    database: Database,
    website: Website,
    login: Any,
) -> None:
    user = _make_user(database, role=UserRole.AUDITOR, email="outsider@example.test")
    login(user)
    response = client.get(f"/api/v1/websites/{website.id}/pages")
    assert response.status_code == 403
    body = response.get_json()
    assert body["type"].endswith("/forbidden")


def test_unknown_website_returns_404(
    client: FlaskClient, admin_user: AppUser, login: Any
) -> None:
    login(admin_user)
    response = client.get("/api/v1/websites/507f1f77bcf86cd799999999/pages")
    assert response.status_code == 404


def test_unknown_page_returns_404(
    client: FlaskClient, admin_user: AppUser, login: Any
) -> None:
    login(admin_user)
    response = client.get("/api/v1/pages/507f1f77bcf86cd799999999")
    assert response.status_code == 404


# ---------------------------------------------------------------------------
# CRUD round-trip
# ---------------------------------------------------------------------------


def test_create_then_get_returns_resource(
    client: FlaskClient,
    admin_user: AppUser,
    website: Website,
    login: Any,
) -> None:
    login(admin_user)
    create = client.post(
        f"/api/v1/websites/{website.id}/pages", json=_valid_create_body()
    )
    assert create.status_code == 201, create.get_json()
    assert create.headers["Location"].startswith("/api/v1/pages/")
    body = create.get_json()
    page_id = body["id"]
    assert body["title"] == "Page one"
    assert body["website_id"] == website.id
    assert body["status"] == "discovered"

    get = client.get(f"/api/v1/pages/{page_id}")
    assert get.status_code == 200
    assert get.get_json()["id"] == page_id


def test_create_with_duplicate_url_returns_existing_page(
    client: FlaskClient,
    admin_user: AppUser,
    website: Website,
    login: Any,
) -> None:
    """create_page is upsert-by-(website_id, url) — second post returns the same id."""
    login(admin_user)
    body = _valid_create_body()
    first = client.post(f"/api/v1/websites/{website.id}/pages", json=body)
    second = client.post(f"/api/v1/websites/{website.id}/pages", json=body)
    assert first.status_code == 201
    assert second.status_code == 201
    assert first.get_json()["id"] == second.get_json()["id"]


def test_patch_updates_only_provided_fields(
    client: FlaskClient,
    admin_user: AppUser,
    website: Website,
    login: Any,
) -> None:
    login(admin_user)
    create = client.post(
        f"/api/v1/websites/{website.id}/pages", json=_valid_create_body()
    )
    page_id = create.get_json()["id"]
    original_url = create.get_json()["url"]

    response = client.patch(
        f"/api/v1/pages/{page_id}", json={"title": "Renamed page"}
    )
    assert response.status_code == 200
    body = response.get_json()
    assert body["title"] == "Renamed page"
    assert body["url"] == original_url
    assert body["priority"] == "normal"


def test_put_replaces_editable_fields(
    client: FlaskClient,
    admin_user: AppUser,
    website: Website,
    login: Any,
) -> None:
    login(admin_user)
    create_body = _valid_create_body()
    create = client.post(
        f"/api/v1/websites/{website.id}/pages", json=create_body
    )
    page_id = create.get_json()["id"]

    new_body = create_body | {"title": "Replaced", "priority": "high"}
    response = client.put(f"/api/v1/pages/{page_id}", json=new_body)
    assert response.status_code == 200
    body = response.get_json()
    assert body["title"] == "Replaced"
    assert body["priority"] == "high"
    # status is server-managed — replace doesn't reset it.
    assert body["status"] == "discovered"


def test_put_rejects_url_change(
    client: FlaskClient,
    admin_user: AppUser,
    website: Website,
    login: Any,
) -> None:
    login(admin_user)
    create_body = _valid_create_body()
    create = client.post(
        f"/api/v1/websites/{website.id}/pages", json=create_body
    )
    page_id = create.get_json()["id"]

    # Different url should be rejected — page identity is (website_id, url).
    response = client.put(
        f"/api/v1/pages/{page_id}",
        json=create_body | {"url": "https://example.test/different"},
    )
    assert response.status_code == 400
    body = response.get_json()
    field_errors = {e["field"] for e in body["errors"]}
    assert "url" in field_errors


def test_delete_returns_204_then_get_returns_404(
    client: FlaskClient,
    admin_user: AppUser,
    website: Website,
    login: Any,
) -> None:
    login(admin_user)
    create = client.post(
        f"/api/v1/websites/{website.id}/pages", json=_valid_create_body()
    )
    page_id = create.get_json()["id"]

    delete = client.delete(f"/api/v1/pages/{page_id}")
    assert delete.status_code == 204
    assert delete.data == b""

    get = client.get(f"/api/v1/pages/{page_id}")
    assert get.status_code == 404


# ---------------------------------------------------------------------------
# Validation
# ---------------------------------------------------------------------------


def test_create_with_missing_url_returns_400(
    client: FlaskClient,
    admin_user: AppUser,
    website: Website,
    login: Any,
) -> None:
    login(admin_user)
    response = client.post(
        f"/api/v1/websites/{website.id}/pages", json={"title": "no url"}
    )
    assert response.status_code == 400
    payload = response.get_json()
    field_errors = {e["field"] for e in payload["errors"]}
    assert "url" in field_errors


def test_create_with_invalid_priority_returns_400(
    client: FlaskClient,
    admin_user: AppUser,
    website: Website,
    login: Any,
) -> None:
    login(admin_user)
    body = _valid_create_body() | {"priority": "urgent"}
    response = client.post(f"/api/v1/websites/{website.id}/pages", json=body)
    assert response.status_code == 400
    payload = response.get_json()
    field_errors = {e["field"] for e in payload["errors"]}
    assert "priority" in field_errors


def test_create_with_non_object_body_returns_400(
    client: FlaskClient,
    admin_user: AppUser,
    website: Website,
    login: Any,
) -> None:
    login(admin_user)
    response = client.post(
        f"/api/v1/websites/{website.id}/pages", json=["not", "a", "dict"]
    )
    assert response.status_code == 400


def test_patch_with_invalid_priority_returns_400(
    client: FlaskClient,
    admin_user: AppUser,
    website: Website,
    login: Any,
) -> None:
    login(admin_user)
    create = client.post(
        f"/api/v1/websites/{website.id}/pages", json=_valid_create_body()
    )
    page_id = create.get_json()["id"]
    response = client.patch(
        f"/api/v1/pages/{page_id}", json={"priority": "urgent"}
    )
    assert response.status_code == 400


# ---------------------------------------------------------------------------
# Pagination + filtering
# ---------------------------------------------------------------------------


def test_list_paginates_with_cursor(
    client: FlaskClient,
    admin_user: AppUser,
    website: Website,
    login: Any,
) -> None:
    login(admin_user)
    for _ in range(3):
        client.post(
            f"/api/v1/websites/{website.id}/pages", json=_valid_create_body()
        )

    page1 = client.get(f"/api/v1/websites/{website.id}/pages?limit=2")
    assert page1.status_code == 200
    page1_body = page1.get_json()
    assert len(page1_body["items"]) == 2
    assert page1_body["next_cursor"] is not None

    cursor = page1_body["next_cursor"]
    page2 = client.get(
        f"/api/v1/websites/{website.id}/pages?limit=2&cursor={cursor}"
    )
    assert page2.status_code == 200
    page2_body = page2.get_json()
    assert len(page2_body["items"]) == 1
    assert page2_body["next_cursor"] is None


def test_list_filters_by_status(
    client: FlaskClient,
    admin_user: AppUser,
    database: Database,
    website: Website,
    login: Any,
) -> None:
    login(admin_user)
    create = client.post(
        f"/api/v1/websites/{website.id}/pages", json=_valid_create_body()
    )
    page_id = create.get_json()["id"]

    # Bypass the API to flip the status to ``tested`` so we can filter.
    page = database.get_page(page_id)
    assert page is not None
    from auto_a11y.models.page import PageStatus
    page.status = PageStatus.TESTED
    database.update_page(page)

    listed_tested = client.get(
        f"/api/v1/websites/{website.id}/pages?status=tested"
    )
    assert listed_tested.status_code == 200
    assert len(listed_tested.get_json()["items"]) == 1

    listed_discovered = client.get(
        f"/api/v1/websites/{website.id}/pages?status=discovered"
    )
    assert listed_discovered.status_code == 200
    assert len(listed_discovered.get_json()["items"]) == 0


def test_list_with_unknown_status_returns_400(
    client: FlaskClient,
    admin_user: AppUser,
    website: Website,
    login: Any,
) -> None:
    login(admin_user)
    response = client.get(
        f"/api/v1/websites/{website.id}/pages?status=fictitious"
    )
    assert response.status_code == 400
    body = response.get_json()
    field_errors = {e["field"] for e in body["errors"]}
    assert "status" in field_errors


def test_list_with_malformed_cursor_returns_400(
    client: FlaskClient,
    admin_user: AppUser,
    website: Website,
    login: Any,
) -> None:
    login(admin_user)
    response = client.get(
        f"/api/v1/websites/{website.id}/pages?cursor=not-a-cursor!@#"
    )
    assert response.status_code == 400


def test_list_isolates_pages_by_website(
    client: FlaskClient,
    admin_user: AppUser,
    database: Database,
    website: Website,
    login: Any,
) -> None:
    login(admin_user)
    other = _make_website(database, _make_project(database))
    client.post(f"/api/v1/websites/{website.id}/pages", json=_valid_create_body())
    client.post(f"/api/v1/websites/{other.id}/pages", json=_valid_create_body())

    listed = client.get(f"/api/v1/websites/{website.id}/pages")
    assert listed.status_code == 200
    items = listed.get_json()["items"]
    assert len(items) == 1
    assert items[0]["website_id"] == website.id
